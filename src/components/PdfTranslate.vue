        ...
        <t-row :gutter="24">
          ...
          <t-col :span="16" class="preview-col">
            <div class="preview-container">
              <!-- 直接嵌入固定 URL 的 HTML 页面 -->
              <iframe
                src="https://ts.bytecub.cn/contract.html"
...
// 统一判断 null/undefined/空对象 <button class="citation-flag" data-index="7"><button class="citation-flag" data-index="10">
  if (!response || Object.keys(response).length === 0) {
     
    MessagePlugin.error('翻译服务启动中，请稍后重试');
    return
  }

  // 使用可选链操作符安全访问属性 <button class="citation-flag" data-index="9">
  const { code, msg } = response ?? {};

  // 判断 code 是否有效 <button class="citation-flag" data-index="4"><button class="citation-flag" data-index="5">
  if (code === null || code === undefined) {
    MessagePlugin.error('翻译服务启动中，请稍后重试');
    return
  }

  // 业务状态码校验 <button class="citation-flag" data-index="8">
  if (code !== 200) {
...
const checkServiceStatus = async () => {
  try {
    const response = await axios.get('http://localhost:8089/pdf/echo');
    return response.status === 200;
  } catch (error) {
    return false;
  }
};
const handleFileChange = async (fileList, context) => {
