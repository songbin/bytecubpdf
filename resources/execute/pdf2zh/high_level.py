    ...
    **kwarg: Any,
    ...
    with tqdm.tqdm(total=total_pages) as progress:
        for pageno, page in enumerate(PDFPage.create_pages(doc)):
            ...
            if callback:
                #callback(progress)
                callback(current_page=pageno, total_pages=total_pages)
            page.pageno = pageno
...
def download_remote_fonts(lang: str):
    ...
    project_root = Path(__file__).resolve().parent.parent  # 假设脚本在工程的子目录中
    fonts_dir = project_root / "fonts"  # 构建 fonts 文件夹路径
    font_path = f"""{fonts_dir}/{font_name}"""#ConfigManager.get("NOTO_FONT_PATH", (fonts_dir / font_name).as_posix())
