...
class BaseTranslator:
    ...
    def translate(self, text: str, ignore_cache: bool = False, rate_limit_params: dict = None) -> str:
        """
    ...
    def prompt(
        ...
        #glossary_str = str(term_dict)  # 将术语表字典转换为字符串形式
        # 构造术语表说明
        # 构造术语表说明（显式区分键值是否相同）
        glossary_str = "\n".join([
            f"- Term: '{key}' → Replace with '{value}' (keep '{key}' UNCHANGED if value == key)." 
            ...
            return [
                    {
                        ...
                        "content": (
                            ...
                            "\n\n"
                            "You are provided with a glossary of terms that must be STRICTLY followed during translation. "
                            "For each term in the glossary: "
                            "1. ONLY replace the EXACT term in the source text with its corresponding value. "
                            "2. If the value is the same as the key (e.g., 'Abstract → Abstract'), keep the term UNCHANGED. "
                            "3. Translate all other content (including text before, after, or adjacent to the term) as usual. "
                            "Do NOT skip or leave any part of the text untranslated unless it is a term in the glossary."
                            "\n\n"
                            "The formula notation {v*} (e.g., {v1}, {v14}) should remain UNCHANGED during translation. "
                            "Treat {v*} as a special placeholder that must not be modified."
                            "\n\n"
                            "Example 1: "
                            "Glossary: 'Abstract → Abstract', Source Text: 'Abstract: This is content.' → Output: 'Abstract: 这是内容。'"
                            "\n\n"
                            "Example 2: "
                            "Glossary: 'enzyme → enzyme', Source Text: 'The enzyme activity...' → Output: 'The enzyme 活性...'"
                            "\n\n"
                            "Example 3: "
                            "Glossary: 'S25DH → S25DH', Source Text: 'S25DH is a molybdenum enzyme.' → Output: 'S25DH 是一种钼酶。'"
                            "\n\n"
...
class OpenAITranslator(BaseTranslator):
    ...
    def do_translate(self, text) -> str:
        msgs =self.prompt(text, self.prompttext)
        logger.info(
            f"发送的消息是: {msgs})"
        )
